package oogasalad.runner.model.action.computer;

import java.util.Map;
import oogasalad.runner.model.board.Board;

/**
 * Computer callback that checks if the player has won the game currently hard coded to check if the
 * player's hand is empty
 */

public class CheckWinAmount implements ComputerAction {
  private final Board board;
  private final int amount;
  public CheckWinAmount(Map<String, String> params, Board board) {
    amount = Integer.parseInt(params.get("amount"));
    this.board = board;
  }
  @Override
  public void act() {
    board.players().stream().filter(player -> player.hand().size() == amount)
        .forEach(player -> player.setWinner(true));
  }

}
